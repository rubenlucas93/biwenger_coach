...
async function test() {
    ...
    try {
        const res = await axios.get(`${client.baseUrl}/rounds/league?fields=*,results`, { headers: client.headers });
        const standings = res.data.data.league.standings;
        console.log("Keys on standing item:", Object.keys(standings[0]));
        console.log("Cancelas:", JSON.stringify(standings.find(u => u.name === "Cancelas")));
